...
public abstract class AbstractDeduplication {
    ...
    protected final GetStateQuery getStateQuery;
    protected final SetStateQuery setStateQuery;
    ...
    // TODO: comment. time in seconds since epoch
    ...
    public AbstractDeduplication(
        ...
        ensureTableExists(bigQuery, properties.getTransactionsTableId());
        ensureTableExists(bigQuery, properties.getStateTableId());
    ...
    protected void runDedupe() {
        try {
            ...
            var result = getDuplicatesTemplateQuery.runWith(
                    timestampWindow.startTimestamp, timestampWindow.endTimestamp);
            // Remove duplicates (if present)
            if (result.getTotalRows() != 0) {
                removeDuplicatesTemplateQuery.runWith(
                        timestampWindow.startTimestamp, timestampWindow.endTimestamp);
            }
    ...
    private static void ensureTableExists(BigQuery bigQuery, TableId tableId) {
