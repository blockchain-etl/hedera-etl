 ...
 *
 * <p>Smart contracts have init bytecode (constructor bytecode) and runtime bytecode (the code
 * executed when the contract is called). This class helps in extracting the runtime bytecode from
 * the given init bytecode by searching for specific patterns.
 */
@UtilityClass
public class BytecodeUtils {
  ...
  /**
   * Compiled regex pattern to match the init bytecode sequence. The pattern checks for a sequence
   * of a free memory pointer setup, a CODECOPY operation, and a RETURN operation, in that order.
   * The sequence is matched case-insensitively to account for hexadecimal representations.
   *
   * <p>Pattern explanation: - (%s|%s) matches either FREE_MEMORY_POINTER or FREE_MEMORY_POINTER_2,
   * which represent setup instructions for the free memory pointer, required for initialization
   * bytecode. - [0-9a-z]+ matches one or more valid hexadecimal characters (0-9, a-f) after the
   * free memory pointer setup. - %s matches the CODECOPY opcode, which copies code to memory and
   * typically follows the memory pointer setup in init bytecode. - [0-9a-z]+ matches one or more
   * valid hexadecimal characters (0-9, a-f) between CODECOPY and RETURN. - %s matches the RETURN
   * opcode, signaling the end of the initialization bytecode.
   *
   * <p>Example pattern: (60806040|60606040)[0-9a-z]+39[0-9a-z]+f3 This example would match any
   * sequence where either "60806040" or "60606040" appears, followed by "39" (CODECOPY) and then
   * "f3" (RETURN), with valid hexadecimal characters in between.
   */
  private static final Pattern INIT_BYTECODE_PATTERN =
      Pattern.compile(
          String.format(
              "(%s|%s)[0-9a-z]+%s[0-9a-z]+%s",
              FREE_MEMORY_POINTER, FREE_MEMORY_POINTER_2, CODECOPY, RETURN),
