...
/**
 * Writes TableRow (serialized to json) and InsertErrors (serialized to json) in BigQueryInsertError
 * to errors table in BigQuery.
 *
 * <p>Failed retry policy is set to always retry since this is last step in failure handling and no
 * failure should be silently ignored. Throwing exception in case of any failure would prompt the
 * runner to retry the whole bundle, and can lead to duplicate inserts. So it is best to let
 * BigQueryIO retry only the failed inserts.
 */
...
class BigQueryErrorsSink extends PTransform<PCollection<BigQueryInsertError>, PDone> {
  ...
  public PDone expand(PCollection<BigQueryInsertError> input) {
    input
        .apply("ConvertErrorToTableRow", ParDo.of(new BigQueryInsertErrorToTableRow()))
        .apply(
            "WriteErrorsToTable",
