...
import com.hedera.etl.reader.recordfile.exception.StreamFileReaderException;
...
public abstract class AbstractPreV5RecordFileReader implements RecordFileReader {
  ...
  public RecordFile read(StreamFilename filename, byte[] bytes) {
    try (RecordFileDigest digest = getRecordFileDigest(new ByteArrayInputStream(bytes));
        ValidatedDataInputStream vdis =
            new ValidatedDataInputStream(digest.getDigestInputStream(), filename.getFilename())) {
      RecordFile recordFile = new RecordFile();
      recordFile.setBytes(bytes);
      recordFile.setDigestAlgorithm(DIGEST_ALGORITHM);
      recordFile.setLoadStart(filename.getTimestamp());
      recordFile.setName(filename.getFilename());
  ...
  /**
   * Reads the record file body, updates the message digest with data from the body, and sets
   * corresponding {@link RecordFile} fields. {@code vdis} should point at the beginning of the
   * body. The body should contain a variable number of transaction and record pairs ordered by
   * consensus timestamp. The body may also contain metadata to mark the boundary of the pairs.
   *
  ...
  private void readBody(
      ValidatedDataInputStream vdis, RecordFileDigest digest, RecordFile recordFile)
      throws IOException {
    int count = 0;
    ...
    while (vdis.available() != 0) {
      ...
      byte[] transactionBytes =
          vdis.readLengthAndBytes(1, MAX_TRANSACTION_LENGTH, false, "transaction bytes");
      byte[] recordBytes =
          vdis.readLengthAndBytes(1, MAX_TRANSACTION_LENGTH, false, "record bytes");
    ...
    final var finalConsensusEnd = consensusEnd;
    items.forEach(item -> item.setFileConsensusEnd(finalConsensusEnd));
  ...
  /**
   * Reads the record file header, updates the message digest with data from the header, and sets
   * corresponding {@link RecordFile} fields. {@code vdis} should point at the beginning of the
   * stream. The header should contain file version, HAPI version, and the previous file hash.
   *
   * @param vdis the {@link ValidatedDataInputStream} of the record file
   * @param recordFile the {@link RecordFile} object
   * @throws IOException
  ...
  protected static class RecordFileDigest implements AutoCloseable {
    ...
    @Getter private final DigestInputStream digestInputStream;
