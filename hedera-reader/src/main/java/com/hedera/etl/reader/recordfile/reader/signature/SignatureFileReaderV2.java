...
public class SignatureFileReaderV2 implements SignatureFileReader {
  ...
  protected static final byte SIGNATURE_TYPE_SIGNATURE =
      3; // the file content signature, should not be hashed
  protected static final byte SIGNATURE_TYPE_FILE_HASH =
      4; // next 48 bytes are SHA-384 of content of record file
    ...
    try (ValidatedDataInputStream vdis =
      ...
      byte[] signature =
          vdis.readLengthAndBytes(
              1,
              StreamFileSignature.SignatureType.SHA_384_WITH_RSA.getMaxLength(),
              false,
              "signature");
      ...
      streamFileSignature.setSignatureType(StreamFileSignature.SignatureType.SHA_384_WITH_RSA);
