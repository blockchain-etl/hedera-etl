...
package com.hedera.etl.reader.recordfile.reader;
...
/**
 * A validated data input stream extends {@link DataInputStream} by providing methods to validate
 * several types of value read from the stream with expected value.
 */
public class ValidatedDataInputStream extends DataInputStream {
  ...
  public byte[] readLengthAndBytes(
      int minLength, int maxLength, boolean hasChecksum, String sectionName, String type)
      throws IOException {
  ...
  public byte[] readNBytes(int expectedLength, String sectionName, String fieldName)
      throws IOException {
  ...
  private String getFullFieldName(String sectionName, String fieldName) {
    List<String> parts =
        Arrays.asList(resourceName, "field", sectionName, fieldName).stream()
  ...
  private <T> T validate(T expected, T actual, String sectionName, String fieldName) {
    if (!Objects.equals(expected, actual)) {
      throw new InvalidStreamFileException(
          String.format(
              NOT_EQUAL_ERROR_MESSAGE, getFullFieldName(sectionName, fieldName), expected, actual));
    }
