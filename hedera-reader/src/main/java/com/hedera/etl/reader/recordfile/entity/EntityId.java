...
public final class EntityId implements Serializable, Comparable<EntityId> {
  ...
  /*
   * Indicates a domain entity ID is not being set (to null or non-null) in the
   * current operation and that the existing DB column value is to be preserved.
   * This is reflected as the value of -1 that can be referenced within
   * the @UpsertColumn syntax. See the AbstractNft delegatingSpender and spender
   * fields as examples. In this case, this sentinel value is set in
   * TokenUpdateNftsTransactionHandler.
   */
  ...
  private static final String CACHE_DEFAULT =
      "expireAfterAccess=60m,maximumSize=500000,recordStats";
  ...
  @Serial private static final long serialVersionUID = 1427649605832330197L;
   ...
   *
   * <p>Only 63 bits (excluding signed bit) are used for encoding to make it easy to encode/decode
   * using mathematical operations too. That's because JavaScript's support for bitwise operations
   * is very limited (truncates numbers to 32 bits internally before bitwise operation).
   *
   * <p>Format: <br>
   ...
   * realm: 0 - 65535 <br>
   * num: 0 - 4294967295 <br>
   * Placing entity num in the end has the advantage that encoded ids <= 4294967295 will also be
   * human-readable.
   */
  private static long encode(long shard, long realm, long num) {
    if (shard > SHARD_MASK
        ...
        || num < 0) {
      log.warn(
          "Invalid entity ID: " + shard + "." + realm + "." + num + " Replacing with EMPTY id");
      return EntityId.EMPTY.getNum();
    }
    ...
    return (num & NUM_MASK)
        | (realm & REALM_MASK) << NUM_BITS
        | (shard & SHARD_MASK) << (REALM_BITS + NUM_BITS);
  }

  @JsonValue private final long id;
