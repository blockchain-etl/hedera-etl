...
public class RecordFileReaderImplV5 implements RecordFileReader {
  ...
  public RecordFile read(StreamFilename filename, byte[] bytes) {
    ...
    // the first DigestInputStream is for file hash and the second is for metadata
    // hash. Any
    // BufferedInputStream
    // should not wrap, directly or indirectly, the second DigestInputStream. The
    // BufferedInputStream after the
    // first DigestInputStream is needed to avoid digesting some class ID fields
    // twice.
    try (DigestInputStream digestInputStream =
            new DigestInputStream(
                new BufferedInputStream(
                    new DigestInputStream(new ByteArrayInputStream(bytes), messageDigestFile)),
                messageDigestMetadata);
        ValidatedDataInputStream vdis =
            new ValidatedDataInputStream(digestInputStream, filename.getFilename())) {
  ...
  private void readBody(
      ValidatedDataInputStream vdis,
      DigestInputStream metadataDigestInputStream,
