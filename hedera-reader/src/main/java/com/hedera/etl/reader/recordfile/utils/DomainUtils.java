...
package com.hedera.etl.reader.recordfile.utils;
...
import com.hedera.etl.reader.recordfile.converter.ObjectToStringSerializer;
import com.hedera.etl.reader.recordfile.entity.EntityId;
import com.hedera.etl.reader.recordfile.exception.InvalidEntityException;
...
public class DomainUtils {
  ...
  static {
    // Ensure it's eagerly instantiated since it is used for the conversion of JSONB
    // data into
    // domain objects.
  ...
  /**
   * Convert bytes to hex and adds prefix 0x.
   *
   * @param bytes to be converted
   * @return converted HexString
   */
  public static String bytesToHexWithPrefix(byte[] bytes) {
    ...
    return ArrayUtils.isNotEmpty(bytes) ? "0x" + Hex.encodeHexString(bytes) : "";
  }
  ...
  /**
   * A key can either be a complex key (e.g. key list or threshold key) or a primitive key (e.g.
   * ED25519 or ECDSA_SECP256K1). If the protobuf encoding of a Key is a single primitive key or a
   * complex key with exactly one primitive key within it, return the key as a String with lowercase
   * hex encoding.
   *
   * @param protobufKey
  ...
  private static byte[] getPublicKey(Key key, int depth) {
    // We don't support searching for primitive keys at multiple levels since the
    // REST API matches
    // by hex prefix
    if (depth > 2) {
      return null;
  ...
  /**
   * Converts time in (second, nanos) to time in only nanos, with a fallback if overflow: If
   * positive overflow, return the max time in the future (Long.MAX_VALUE). If negative overflow,
   * return the max time in the past (Long.MIN_VALUE).
   */
  ...
  /**
   * Converts instant to time in only nanos, with a fallback if overflow: If positive overflow,
   * return the max time in the future (Long.MAX_VALUE). If negative overflow, return the max time
   * in the past (Long.MIN_VALUE).
   */
  ...
  public static Long timeStampInNanos(Timestamp timestamp) {
    try {
      ...
      return Math.addExact(
          Math.multiplyExact(timestamp.getSeconds(), NANOS_PER_SECOND), timestamp.getNanos());
    } catch (ArithmeticException e) {
      throw new ArithmeticException(
          "Long overflow when converting Timestamp to nanos timestamp: " + timestamp);
    }
  }
  ...
  /**
   * Cleans a string of invalid characters that would cause it to fail when inserted into the
   * database. In particular, PostgreSQL does not allow the null character (0x0000) to be inserted.
   *
  ...
  /**
   * This method converts a protobuf ByteString into a byte array. Optimization is done in case the
   * input is a LiteralByteString to not make a copy of the underlying array and return it as is.
   * This is okay for our purposes since we never modify the array and just directly store it in the
   * database.
   *
   * <p>If the ByteString is smaller than the estimated size to allocate an UnsafeByteOutput object,
   * copy the array regardless since we'd be allocating a similar amount of memory either way.
   *
  ...
  // The 'shard.realm.num' form evm address has 4 bytes for shard, and 8 bytes
  // each for realm and
  // num.
  ...
  public static String toBase32(ByteString bytes) {
    return toBase32(toBytes(bytes));
  }
  ...
  static class UnsafeByteOutput extends ByteOutput {
    ...
    static final short SIZE =
        12 + 4; // Size of the object header plus a compressed object reference to bytes field
    ...
    static {
      try {
        ...
        throw new ProtobufException(
            String.format(
                "Unable to locate class=%s", ByteString.class.getName() + "$LiteralByteString"));
      }
    }
