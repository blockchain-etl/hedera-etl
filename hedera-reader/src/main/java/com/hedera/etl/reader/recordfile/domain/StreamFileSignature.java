...
import com.hedera.etl.reader.recordfile.domain.addressbook.ConsensusNode;
import com.hedera.etl.reader.recordfile.utils.DomainUtils;

import static com.hedera.etl.reader.recordfile.reader.signature.ProtoSignatureFileReader.VERSION;
...
@NoArgsConstructor
@ToString(
    exclude = {"bytes", "fileHash", "fileHashSignature", "metadataHash", "metadataHashSignature"})
public class StreamFileSignature implements Comparable<StreamFileSignature>, Serializable {

  private static final String COMPRESSED_EXTENSION = ".gz";
  private static final Comparator<StreamFileSignature> COMPARATOR =
      Comparator.comparing(StreamFileSignature::getNode)
  ...
  @EqualsAndHashCode.Include private StreamFilename filename;

  private byte[] metadataHash;
  private byte[] metadataHashSignature;

  @EqualsAndHashCode.Include private ConsensusNode node;

  private SignatureType signatureType;

  @Builder.Default private SignatureStatus status = SignatureStatus.DOWNLOADED;
  ...
  public enum SignatureStatus {
    ...
    CONSENSUS_REACHED, // Signature verification consensus reached by a node count greater than the
    // consensusRatio
